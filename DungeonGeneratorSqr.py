...
parser.add_argument('-r', '--rooms', type=int, default=20, help='rooms in dungeon')
...
class Room:
    ...
    def expand_sqr(self, size):
        new_pos = random.choice(list(self.allowed_new_block_positions()))
        # print(new_pos)
        nx, ny = new_pos.x, new_pos.y
        for ix in range(size):
            for iy in range(size):
                new_block = Block(Position(nx + ix, ny + iy))
...
class Dungeon:
    ...
    def create_room(self, blocks, doors):
        ...
        if ROOM_SQR:
            room.expand_sqr(int(random.randint(*blocks) ** 0.5)+1)
        else:
